...
const char *mwa_gpu_analytic_calc_jones(
    const ANALYTIC_TYPE at, const FLOAT dipole_height_m, const FLOAT *d_azs,
...
__global__ void mwa_analytic_kernel(
    const ANALYTIC_TYPE at, const FLOAT dipole_height_m, const FLOAT *azs,
    const FLOAT *zas, const int num_directions, const unsigned int *freqs_hz,
    const int num_freqs, const FLOAT *delays, const FLOAT *amps,
    const int num_tiles, const FLOAT latitude_rad, const bool norm_to_zenith,
    const uint8_t bowties_per_row, JONES *results) {
      ...
      for (int i_freq = 0; i_freq < num_freqs; i_freq++) {
        ...
        for (int row = 0; row < bowties_per_row; row++) {
          for (int col = 0; col < bowties_per_row; col++) {
            ...
            FLOAT phase = 0.0;
            FLOAT delay =
                delays[i_tile * num_bowties + row * (int)bowties_per_row + col];
            ...
            FLOAT amp =
                amps[i_tile * num_bowties + row * (int)bowties_per_row + col];
            array_factor += MAKE_COMPLEX(c_phase, s_phase) * amp;
          }
        }

        FLOAT ground_plane = 2.0 *
                             SIN(M_2PI * dipole_height_m / lambda_m * c_za) /
                             (FLOAT)num_bowties;
        if (norm_to_zenith) {
        ...
        results[((num_directions * num_freqs * i_tile) +
...
extern "C" const char *mwa_gpu_analytic_calc_jones(
    const ANALYTIC_TYPE at, const FLOAT dipole_height_m, const FLOAT *d_azs,
    const FLOAT *d_zas, int num_directions, const unsigned int *d_freqs_hz,
    const int num_freqs, const FLOAT *d_delays, const FLOAT *d_amps,
    const int num_tiles, const FLOAT latitude_rad, const uint8_t norm_to_zenith,
  ...
  mwa_analytic_kernel<<<gridDim, blockDim>>>(
      at, dipole_height_m, d_azs, d_zas, num_directions, d_freqs_hz, num_freqs,
      d_delays, d_amps, num_tiles, latitude_rad, (bool)norm_to_zenith,
      bowties_per_row, (JONES *)d_results);
